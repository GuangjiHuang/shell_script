...
# if exists the record_path.txt, and then backup it
...
# then copy the record_path.txt to the file from the backup
if [ -f "/opt/myscript/backup/record_path.txt" ]; then
    cp /opt/myscript/backup/record_path.txt /opt/myscript/p-script/
    echo "--recover the record_path.txt sucessfully!"
fi
