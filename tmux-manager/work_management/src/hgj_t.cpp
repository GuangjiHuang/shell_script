...
GET_LINE:
    ...
    if (read_line && *readline) {
        add_history(read_line);
        item = string(read_line);
        free(read_line);
    }
    else {
        cout << endl << "[ warnning ]: NULL readline" << endl;
        goto GET_LINE;
    }
...
/* class Path_man */ 
// public function
Path_man::Path_man(string parent_dir, const string& file_type) : parent_dir(parent_dir), file_type(file_type) {
    // init the date_str
...
bool Path_man::updateFileType(string file_type) {
    ...
    if (defined_file_type.count(file_type)) {
        ...
        if (! _mkCurPath()) { // has not such file
