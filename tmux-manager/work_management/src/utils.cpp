...
void printExe(const string& exeinfo) {
    string x_x = CCOLOR(LIGHT_GREEN, "[^_^]: ");
    x_x += exeinfo;
    cout << x_x << endl;
}
...
void printContent(const File_t& file, const Show_arg& show_arg, bool is_just_show_title) {
    ...
    stringstream ss_try;
    string str_ss_try;
    int show_number_count = 0;
    while (--state_show_num>=0 && items_it!=str_to_be_print.end()) {
        //cout << *items_it++ << endl;
        ss_try << *items_it++ << "\n";
        ++show_number_count;
    }
    ss_try << hyphen(hyphen_num) << endl;
    str_ss_try = ss_try.str();
    // if the string too long, use the echo and the less to show the result 
    if (show_number_count > 15) {
        string str_command = string("echo ") + "\"" + str_ss_try + "\" | less -n";
        system(str_command.c_str());
    }
    else {
        cout << str_ss_try << endl;
    }
}
...
string printPromptStudy(const StudyT& study, bool is_toggle) {
    ...
    prompt = "[" + prompt + "] "; // the command part
    prompt = SCOLOR(LIGHT_GREEN, prompt);
    // get the information: topic1, topic2, and the question_id
    string t1_str = study.topic1.empty()? "*" : study.topic1;
    string t2_str = study.topic2.empty()? "*" : study.topic2;
    string qid_str = study.question_id==-1 ? "*" : to_string(study.question_id);
    string info_str;
    {
        stringstream ss;
        string link_mark = study.link.empty() ? " !" : " *";
        ss << "(" << t1_str << "@" << t2_str << " - " << qid_str << link_mark << ")";
        info_str = ss.str();
        info_str = SCOLOR(YELLOW, info_str);
        info_str += CCOLOR(LIGHT_BLUE, " $: ");
    }
    prompt += info_str;
    cout << endl;
    //cout << prompt;
...
void renewStudyPrompt(string&prompt, const string&add_content, const char mark) {
    auto it = prompt.find_first_of(mark);
    if (it!=string::npos) {
        string new_prompt = prompt.substr(0, it) + add_content + prompt.substr(it-1);
        prompt = new_prompt;
    }
}
...
void natureSort(json& questions_key_val, vector<string>& questions_vec) {
    questions_vec.clear();
    for (auto it=questions_key_val.begin(); it!=questions_key_val.end(); ++it) {
        questions_vec.emplace_back(it.key());
    }
    // sort 
    auto sort_reg = [&](string&a, string&b) {
        int id_a = questions_vec.size() - 1;
        int id_b = questions_vec.size() - 1;
        try {
            id_a = stoi(a);
            id_b = stoi(b);
        }
...
bool parseRange(string& range_str, vector<int>& vec_num) {
    vec_num.clear();
    vector<string> str_ls;
    vector<int> range_ls;
    Parser::split(range_str, str_ls, ":");
    for (auto& ele : str_ls) {
        if (! isNum(ele)) {
            range_ls.clear();
            return false;
        }
        range_ls.emplace_back(stoi(ele));
    }
    if (range_ls.size() == 1) {
        ;
    }
    else if (range_ls.size() == 2) {
        ;
    }
    else if (range_ls.size() == 3) {
        ;
    }
    else {
        ;
